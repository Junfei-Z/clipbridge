# ClipBridge v0.6 public web entry

The static app in `web/` is an early v0.6 foundation designed for the GitHub project-page URL:

```text
https://junfei-z.github.io/clipbridge/
```

It is deliberately a public entry point rather than a relay server. GitHub Pages serves only the installable interface. The work remains parked until v0.4.1 one-to-many delivery and the v0.5 Mac relay/client-role model are complete. In the current Web Foundation preview, the page detects the current device, validates a private-network pairing URL, and performs a top-level navigation to the user's own ClipBridge Hub.
